    ...
    >
      ...
      >
        ...
        <div className={`mb-6 ${highlight("contact")}`}>
          ...
          <div className="text-blue-50 space-y-1.5" style={{ fontSize: "var(--font-small)" }}>
        ...
        {(content.skills.technical?.length ?? 0) > 0 && (
          <div className={`mb-6 ${highlight("skills")}`}>
            <h2 className="font-bold uppercase tracking-wider text-white mb-3 pb-1 border-b border-blue-400" style={{ fontSize: "var(--font-header)" }}>
              Technical Skills
            </h2>
            {content.skills.technical.map((group) => (
              <div key={group.id} className="mb-3 last:mb-0">
                <h3 className="uppercase text-blue-100 mb-1.5" style={{ fontSize: "var(--font-small)" }}>{group.header}</h3>
        ...
        {content.languages.length > 0 && (
          <div className={`mb-6 ${highlight("languages")}`}>
            ...
            {content.languages.map((lang) => (
              <div key={lang.id} className="mb-1.5" style={{ fontSize: "var(--font-small)" }}>
            ...
            {content.certifications.map((cert) => (
              <div key={cert.id} className="mb-2" style={{ fontSize: "var(--font-small)" }}>
                <div className="font-medium">
                  {cert.name}
                  {cert.url && (
                    <a href={cert.url} target="_blank" rel="noopener noreferrer" className="text-blue-200 ml-1 hover:underline" style={{ fontSize: "var(--font-xs)" }}>
                      [Link]
                    </a>
                  )}
                </div>
                {cert.issuer && <div className="text-blue-100">{cert.issuer}</div>}
                {cert.date && <div className="text-blue-100">{cert.date}</div>}
              </div>
            ))}
          </div>
        )}
                    ...
                    <div key={exp.id} className="mb-3">
                      <div className="flex justify-between items-baseline mb-1">
                        <div>
                          ...
                          <span className="text-blue-600 ml-1">@ {exp.company}</span>
                        </div>
                ...
                <section key={sectionKey} className={`mb-4 ${highlight("skills")}`}>
                  ...
                  {content.skills.technical.map((group) => (
                    <div key={group.id} className="mb-2">
                      <h3 className="uppercase text-blue-600 mb-1" style={{ fontSize: "var(--font-small)" }}>{group.header}</h3>
                      <div className="flex flex-wrap gap-1">
                        {group.items.map((skill, i) => (
                          <span
                            key={i}
                  ...
                  {content.projects.map((proj) => (
                    <div key={proj.id} className="mb-2.5">
                      <div className="font-semibold text-gray-900">
                        {proj.name}
                        {proj.url && (
                          <a href={proj.url} target="_blank" rel="noopener noreferrer" className="text-blue-600 ml-1 hover:underline" style={{ fontSize: "var(--font-small)" }}>
                            [Link]
                          </a>
                        )}
                      </div>
                      ...
                      {proj.technologies.length > 0 && (
                        <p className="text-blue-600 mt-0.5" style={{ fontSize: "var(--font-xs)" }}>
                  ...
                  {content.certifications.map((cert) => (
                    <div key={cert.id} className="mb-2" style={{ fontSize: "var(--font-body)" }}>
                      <div className="font-medium">
                        {cert.name}
                        {cert.url && (
                          <a href={cert.url} target="_blank" rel="noopener noreferrer" className="text-blue-600 ml-1 hover:underline" style={{ fontSize: "var(--font-xs)" }}>
                            [Link]
                          </a>
                        )}
                      </div>
